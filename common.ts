...
import { readableStreamToString, LambdaError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new LambdaError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new LambdaError(e, e.response?.data)
                }
                if (innerError instanceof LambdaError) {
                    // Got "LambdaError" from the above try block
                    throw innerError;
                }
